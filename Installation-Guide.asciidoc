...
=== Virus scanner
Zanata is tested with clamav's virus scanner (`clamd`/`clamdscan`).  If clam is not installed, a warning will be logged when files are uploaded.  If clam is installed but `clamd` is not running, Zanata may reject all uploaded files (depending on file type).  To install clam:
----
