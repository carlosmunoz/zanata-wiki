...
ln -s /usr/share/java/mysql-connector-java.jar {deploymentsDir}/mysql-connector-java.jar
----
...
=== Fonts 
Some administration functions (ie JavaMelody) are partially localized, so you may need to install, for
...
[NOTE]
By default, the filename of the war file in {deploymentsDir} determines the URL of your zanata server.
...
In +{standaloneXml}+, search subsystem `xmlns="urn:jboss:domain:naming:{namingVer}"` and add bindings as following. Adjust the value accordingly. 
[source,xml]
<subsystem xmlns="urn:jboss:domain:naming:{namingVer}">
    <bindings>
        <simple name="java:global/zanata/security/auth-policy-names/internal" value="zanata.internal"/> # <1>
        <simple name="java:global/zanata/security/admin-users" value="admin"/>                          # <2>
        <simple name="java:global/zanata/email/default-from-address" value="no-reply@zanata.org"/>      # <3>
        <simple name="java:global/zanata/files/document-storage-directory" value="/example/path"/>      # <4>
   </bindings>
    <remote-naming/>
</subsystem>

<1> Replace +zanata.internal+ with your authentication type.
<2> Replace +admin+ with the lists of users that will become the admin once they finished registration. Use with care!
<3> Replace +no-reply@zanata.org+ with the email address you want your user to see as "From:".
<3> Replace +/example/path+ with the absolute path to the directory for document storage.
